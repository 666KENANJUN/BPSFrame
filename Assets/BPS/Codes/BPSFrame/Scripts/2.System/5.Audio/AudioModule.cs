...
namespace BPSFrame
{
    public class AudioModule : MonoBehaviour
    {
        ...
        public void PlayBGAudio(string clipName, bool loop = true, float volume = -1, float fadeOutTime = 0, float fadeInTime = 0)
        {
            IsLoop = loop;
            if (volume != -1)
            {
            ...
            AudioClip clip = ResSystem.LoadAsset<AudioClip>(clipName);
            fadeCoroutine = StartCoroutine(DoVolumeFade(clip, fadeOutTime, fadeInTime));
        }
        ...
        public void PlayOneShot(string clipName, Component component = null, bool autoReleaseClip = false, float volumeScale = 1, bool is3d = true, Action callBack = null)
        {
            // 初始化音乐播放器
            AudioSource audioSource = GetAudioPlay(is3d);
            if (component == null) audioSource.transform.SetParent(null);
