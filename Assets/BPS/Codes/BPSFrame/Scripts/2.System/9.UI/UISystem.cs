...
namespace BPSFrame
{
    ...
    public class UISystem : MonoBehaviour
    {
        ...
        {
            instance = BPSFrameRoot.RootTransform.GetComponentInChildren<UISystem>();
        }
        #region 内部类
        [Serializable]
        ...
        private static Dictionary<string, UIWindowData> UIWindowDataDic => BPSFrameRoot.Setting.UIWindowDataDic;
        ...
        public static void AddUIWindowData(string windowKey, UIWindowData windowData, bool instantiateAtOnce = false)
        {
            if (UIWindowDataDic.TryAdd(windowKey, windowData))
            {
                if (instantiateAtOnce)
                {
                    ...
                    {
                        BPSLog.Warning("JKFrame:UIWindowData中的isCache=false，但instantiateAtOnce=true!提前实例化对于不需要缓存的窗口来说没有意义");
                    }
                }
            }
        }
        ...
        public static UI_WindowBase Show(string windowKey, int layer = -1)
        {
            ...
            // 资源库中没有意味着不允许显示
            BPSLog.Log($"JKFrame:不存在{windowKey}的UIWindowData");
            return null;
        }
        ...
        public static void Close(string windowKey)
        {
            if (TryGetUIWindowData(windowKey, out UIWindowData windowData))
            {
                ...
                else BPSLog.Warning("JKFrame:您需要关闭的窗口不存在或已经关闭");
            }
            else BPSLog.Warning("JKFrame:未查询到UIWindowData");
        }
